...
| 사용 기술 | 프론트엔드는 React 또는 React Native를 사용하여 사용자 인터페이스를 구현한다. 백엔드는 Node.js 기반 서버를 활용하여 데이터 처리와 API를 관리한다. AI 기능은 OpenAI API를 활용하여 대화 요약, 키워드 추출, 인사이트 정리를 수행한다. 인사이트 간의 관련성을 분석하기 위해 텍스트 임베딩을 생성하고 벡터 유사도 계산을 통해 관련 질문을 추천하는 기능을 구현한다. 데이터 저장은 Firebase 또는 Supabase와 같은 클라우드 데이터베이스를 활용한다. |
